...
assessment <- list(name = "SC_{IQR}", assess = function(data) {
    axis_score <- function(data) {
        ...
        for (xb in 1:(length(breaks_x) - 1)) {
            ...
            x_bin_iq_ranges <- append(x_bin_iq_ranges,
        ...
        y <- data[,2]
        overall_variance <- IQR(y)

        # Find maximum discrepancy between a partition and the whole plot.

        max_diff <- 0
        ncomparisons <- length(x_bin_iq_ranges)
        for (i in 1:ncomparisons) {
            diff_variance <- abs(overall_variance - x_bin_iq_ranges[i])
